<?php
  ...
  if(isset($_GET['proId'])){
    $project = $myPDO->query("SELECT * FROM projects WHERE id = ".$_GET['proId']);
    $projectRow = $project->fetch();
    if($projectRow){
      $mapSize = $projectRow['mapSize'];
      $objects = $projectRow['objects'];
    }
  }else{
    if(isset($_GET['mapWidth']) && isset($_GET['mapHeight'])){
      $mapSize = json_encode([$_GET['mapWidth'], $_GET['mapHeight']]);
      $objects = null;
    }else{
...
<body>
  ...
  <div id="miniMap"></div>
  <div id="arrowBox">
    <div class="arrows" id="topArrow"></div>
    <div class="arrows" id="rightArrow"></div>
    <div class="arrows" id="bottomArrow"></div>
    <div class="arrows" id="leftArrow"></div>
    <div class="arrows" id="leftRotate"><code>&#10556;</code></div>
    <div class="arrows" id="rightRotate"><code>&#10557;</code></div>
  </div>
  <div id="lockCameraButton" class="notActive">
    <div id="circleCameraButton"></div>
  </div>
  <div id="placementGridCont">
  ...
  <script type="module">
      ...
      var roomWidth = Number(<?php echo $mapSize; ?>[0]);
      var roomLength = Number(<?php echo $mapSize; ?>[1]);
      ...
      function getSavedLayout(listOfSavedObj){
        if(listOfSavedObj != null){
          for(var i = 0; i < listOfSavedObj.length; i++){
            objectAdd(listOfSavedObj[i][0], listOfSavedObj[i][1], listOfSavedObj[i][2], listOfSavedObj[i][3], listOfSavedObj[i][4], listOfSavedObj[i][5]);
          }
        }
      }

      getSavedLayout(<?php echo $objects; ?>);

      function saveLayout(){
        // save layout in folder
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "./processing.php?" + "userId=" + <?php echo $_GET['userId'];?> +"&mapSize=" + encodeURIComponent(JSON.stringify([roomWidth,roomLength])) + "&objects=" + encodeURIComponent(JSON.stringify(allAddedGLTFObjects)) + "&proId=" + <?php if(isset($_GET['proId'])){echo $_GET['proId'];}else{ echo -1;} ?> , true);
        xhr.onload = function(){
            if(this.status == 200){
              console.log(this.responseText);
              window.location.replace("./home.php?id="+ <?php echo $_GET['userId'];?>);
            }
        }
      ...
      document.getElementById("saveProject").addEventListener("click", saveLayout);
      ...
      button.addEventListener('click', function() {
        ...
        if (button.classList.contains("notActive")) {
          ...
          document.getElementById("saveProject").style.display = "flex";
          selectorUi.style.display = "none";
      ...
      function gltfSelectorFun(e) {
        ...
        for(var j = 0; j < selectorUi.children.length; j++){
          if(selectorUi.children[j] == gltfImage){
        ...
        selectorUi.append(gltfImage.cloneNode(true));
